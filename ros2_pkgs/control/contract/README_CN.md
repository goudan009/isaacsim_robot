...
当前 Isaac runtime 目标是 Isaac Sim 5.1。这个包只提供共享 contract，不实现
...
- `runtime` 和 `domains` 固定 Isaac Sim 5.1、真机 ROS domain 0、仿真 ROS domain 1；
  同时保留 `robot` 中的同名兼容字段。
- 每个 `actions` 条目定义 controller、topic、字段顺序、单位、逐字段范围、默认命令频率
  与 `clip_each_field_to_limits` 语义。关节范围来自生成后的 OpenFleX URDF；频率来自
...
默认 verifier 会读取 `isaacsim_bringup` 的 controller/sensor YAML 对照配置。
因此应编译并 source contract、description 和 bringup 三个包后运行：

```bash
export OPENFLEX_WS=/path/to/OpenFlex_ws
cd "$OPENFLEX_WS"
source ~/.bashrc
...
ros2 run isaacsim_embodiment_contract verify_embodiment_contract.py
colcon test --packages-select isaacsim_embodiment_contract --event-handlers console_direct+
```
...
如果只 build/source `isaacsim_embodiment_contract`，安装布局测试仍可验证 contract
自身的安装产物，但默认 verifier 会提示缺少 `isaacsim_bringup` 配置。此时可
显式传入配置文件：
...
下一阶段是工业任务 adapter：使用这个 contract 对齐 Isaac Sim 5.1 里的机器人动作、
