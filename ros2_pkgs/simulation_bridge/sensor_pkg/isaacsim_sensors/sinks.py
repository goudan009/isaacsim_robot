    ...
    def attach_cameras(self, cameras: list[dict[str, Any]]) -> object:
        ...
        for index, camera in enumerate(cameras):
            ...
            if exec_gate_step > 0 and not graph_owned:
                for sensor_node in tuple(sensor_nodes) + ((info_node,) if publish_camera_info else ()):
            ...
            if graph_owned:
                ...
                render_nodes = tuple(sensor_nodes) + ((info_node,) if publish_camera_info else ())
            ...
            if publish_depth:
                sensor_specs.append((depth_node, camera.get("depth_topic", "depth/image_rect_raw"), "depth"))
            for node, topic, sensor_type in sensor_specs:
                set_values.extend([
    ...
    def configure_camera_gates(self, cameras: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
        ...
        for index, camera in enumerate(cameras):
            ...
            if not use_srtx:
                ...
                if bool(camera.get("publish_camera_info", True)):
