...
def _enable_stable_ids() -> None:
    ...
    try:
        ...
        print(f"[isaacsim_sensors] Stable ID setting unavailable: {exc}", flush=True)


def _resolve_mid360_config() -> str:
    """Resolve the historical config through the shared Isaac helper when present."""
    try:
        import launch_sensor

        return str(launch_sensor._resolve_lidar_config("Example_Rotary", 3))
    except (ImportError, AttributeError):
        # ``Example_Rotary`` is the Isaac Sim 6 packaged fallback. This keeps
        # the consolidated package usable in a bare SimulationApp worker.
...
def _set_lidar_attributes(stage: object, lidar_path: str, attributes: dict[str, object]) -> None:
    ...
    for name, value in attributes.items():
        ...
        if attribute.IsValid():
            ...
            print(f"[isaacsim_sensors] MID360 attribute unavailable: {name}", flush=True)


def _configure_mid360_performance_profile(stage: object, lidar_path: str) -> None:
    """Use the validated approximately 30k-point performance profile."""
...
def _create_lidar(stage: object, lidar_path: str, *, tick_rate_hz: float = 10.0) -> object:
    """Create the packaged Example_Rotary lidar used by the old repository."""
    from isaacsim.sensors.experimental.rtx import Lidar

    config = _resolve_mid360_config()
    try:
        lidar = Lidar.create(
            path=lidar_path,
            config=config,
...
def create_standalone_mid360(
    ...
    *,
    ...
    print(
        f"[isaacsim_sensors] Created standalone MID360 profile={requested_profile}, "
    ...
    def __init__(self, stage: object, mount_path: str, root_path: str) -> None:
        ...
        if os.environ.get("OPENFLEX_MID360_UNSAFE_FOLLOW", "").strip().lower() in {
            ...
            try:
                ...
                print(
                    "[isaacsim_sensors] WARNING: unsafe MID360 runtime follow enabled; "
                    "this may race RTX rendering",
                    flush=True,
                )
            except Exception as exc:  # pragma: no cover - depends on Isaac runtime
                print(
                    "[isaacsim_sensors] unsafe MID360 follow unavailable; "
...
def _print_robot_mid360_diagnostics(stage: object, mount_path: str, root_path: str, lidar_path: str) -> None:
    ...
    try:
        ...
        print(
            "[isaacsim_sensors] MID360 diagnostics: "
            + "; ".join(
                (
        ...
        if lidar and lidar.IsValid():
            for attribute_name in (
                ...
                if attribute.IsValid():
                    print(
                        f"[isaacsim_sensors] MID360 attribute {attribute_name}="
                        f"{attribute.Get()}",
                        flush=True,
                    )
        if root and root.IsValid():
            print(
                "[isaacsim_sensors] MID360 root xform ops: "
                + ", ".join(
        ...
        print(f"[isaacsim_sensors] MID360 diagnostics failed: {type(exc).__name__}: {exc}", flush=True)
...
def create_robot_mid360(
    ...
    *,
    ...
    mount_mode: str = "fixed_kinematic",
    object_id_map: bool = True,
    tick_rate_hz: float = 10.0,
) -> str:
    """Create a robot MID360 using the historical ROS helper chain.

    ``fixed_kinematic`` is the safe default: it copies the robot mount pose
    once and gives the stage-level sensor root the same kinematic/mass APIs as
    the validated standalone MID360. ``fixed`` copies the pose without those
    physics APIs. ``follow`` is retained as an experimental compatibility mode;
    it only follows at runtime when ``OPENFLEX_MID360_UNSAFE_FOLLOW=1`` is set.
    """
    import omni.graph.core as og
    from isaacsim.sensors.experimental.rtx import Lidar
    ...
    if mount_mode not in {"follow", "fixed_kinematic", "fixed"}:
        raise ValueError("MID360 mount_mode must be 'follow', 'fixed_kinematic', or 'fixed'")
    if tick_rate_hz <= 0.0:
    ...
    lidar_path = str(lidar.paths[0])
    if lidar.prims[0] is None:
        raise RuntimeError(f"failed to create MID360 at {lidar_path}")
    if requested_profile in {"MID360_PERFORMANCE", "LIVOX_MID360_PERFORMANCE"}:
        _configure_mid360_performance_profile(stage, lidar_path)
    else:
        _configure_mid360_full_profile(stage, lidar_path)
    ...
    if transport == "native":
        ...
        print(
            f"[isaacsim_sensors] Created MID360 native PointCloud2 writer at {render_product_path}, "
            f"topic={topic}",
            flush=True,
        )
        return lidar_path
    ...
    graph, _, _, _ = og.Controller.edit(
        {"graph_path": graph_path, "evaluator_name": "execution"},
        {
            keys.CREATE_NODES: [
                ("OnPlaybackTick", "omni.graph.action.OnPlaybackTick"),
                ("ROS2Context", "isaacsim.ros2.bridge.ROS2Context"),
                ("PCLLidarConfig", "isaacsim.ros2.bridge.ROS2RtxLidarPointCloudConfig"),
            ...
            keys.CONNECT: [
                ...
                (
                    "PCLLidarConfig.outputs:selectedMetadata",
                    "RtxLidarHelper.inputs:selectedMetadata",
                ),
            ],
            keys.SET_VALUES: [
                ("RtxLidarHelper.inputs:renderProductPath", render_product_path),
                ("RtxLidarHelper.inputs:topicName", topic),
                ("RtxLidarHelper.inputs:frameId", frame_id),
                # Match the validated standalone helper contract. In
                # particular, do not let a default namespace or stop-time
                # simulation reset alter the RTX writer's topic/timestamp
                # state when the sensor is attached after stage startup.
                ("RtxLidarHelper.inputs:nodeNamespace", ""),
                ("RtxLidarHelper.inputs:resetSimulationTimeOnStop", False),
                ("RtxLidarHelper.inputs:type", "point_cloud"),
                ("RtxLidarHelper.inputs:fullScan", True),
                ("PCLLidarConfig.inputs:outputIntensity", True),
                ("PCLLidarConfig.inputs:outputTimestamp", True),
                ("PCLLidarConfig.inputs:outputChannelId", True),
                ("PCLLidarConfig.inputs:outputEmitterId", True),
                ("PCLLidarConfig.inputs:outputObjectId", bool(object_id_map)),
                (
                    "RtxLidarHelper.inputs:frameSkipCount",
                    max(0, int(os.environ.get("OPENFLEX_LIDAR_FRAME_SKIP", "0"))),
                ),
            ],
        },
    )
    if object_id_map:
        # The object-id map writer is optional.  Keeping it out of the graph
        # avoids initializing a second RTX metadata product in the combined
        # camera + MID360 process when the point cloud itself is sufficient.
        graph, _, _, _ = og.Controller.edit(
            {"graph_path": graph_path, "evaluator_name": "execution"},
            {
                keys.SET_VALUES: [
                    ("RtxLidarHelper.inputs:enableObjectIdMap", True),
                    (
                        "RtxLidarHelper.inputs:objectIdMapTopicName",
                        topic.rstrip("/") + "/object_id_map",
                    ),
                ]
            },
        ...
        print(
            "[isaacsim_sensors] MID360 object-id map disabled for combined sensor run",
            flush=True,
        )
    og.Controller.evaluate_sync(graph)
    _print_robot_mid360_diagnostics(stage, parent_path, root_path, lidar_path)
    print(
        f"[isaacsim_sensors] Created MID360 at {lidar_path}, graph={graph_path}, "
